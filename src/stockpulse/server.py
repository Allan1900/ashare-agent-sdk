"""OpenAI-compatible FastAPI server with indicator + report support."""
...
def _route_query(text: str) -> str:
    ...
    try:
        # ── Report ──
        if any(kw in t for kw in ["分析报告", "综合分析", "全面分析", "深度分析", "研报"]):
            if not code:
                return "请提供股票代码或名称"
            return analyze(code)
        ...
        if any(kw in t for kw in ["macd", "dif", "dea"]):
            if not code: return "请提供股票代码或名称"
            df = query_daily(code, max(days * 2, 120))
            if df.empty: return f"无数据: {code}"
            from .indicators import calc_macd
            return fmt_df(calc_macd(df.sort_values("trade_date")).tail(min(days * 2, 30)))
        ...
        if any(kw in t for kw in ["rsi", "超买", "超卖"]):
            ...
            return fmt_df(calc_rsi(df.sort_values("trade_date")).tail(min(days * 2, 30)))

        if any(kw in t for kw in ["kdj", "随机"]):
            if not code: return "请提供股票代码或名称"
            df = query_daily(code, max(days * 2, 60))
            if df.empty: return f"无数据: {code}"
            from .indicators import calc_kdj
            return fmt_df(calc_kdj(df.sort_values("trade_date")).tail(min(days * 2, 30)))

        if any(kw in t for kw in ["boll", "布林"]):
            if not code: return "请提供股票代码或名称"
            df = query_daily(code, max(days * 2, 60))
            if df.empty: return f"无数据: {code}"
            from .indicators import calc_boll
            return fmt_df(calc_boll(df.sort_values("trade_date")).tail(min(days * 2, 30)))

        if "金叉" in t:
            if not code: return "请提供股票代码或名称"
            from .indicators import check_golden_cross
            sig = check_golden_cross(code)
            return sig or "无金叉信号"

        if "死叉" in t:
            if not code: return "请提供股票代码或名称"
            from .indicators import check_death_cross
            sig = check_death_cross(code)
            return sig or "无死叉信号"
        ...
        if query_type == "daily":
            if code: return fmt_df(query_daily(code, days))
            return "请提供股票代码或名称"
        elif query_type == "moneyflow":
            if code: return fmt_df(query_moneyflow(code, days))
            return "请提供股票代码或名称"
        elif query_type == "financial":
            if code: return fmt_df(query_financial(code))
            return "请提供股票代码或名称"
            ...
            if trade_date: return fmt_df(query_top_list(trade_date))
            return "请提供日期，如 龙虎榜 20260724"
        elif query_type == "macro":
            for kw in ["cpi", "ppi", "pmi", "gdp", "m2", "shibor", "lpr"]:
                if kw in t: return fmt_df(query_macro(kw))
            ...
            for kw in ["搜索", "查找", "找股票"]:
                if kw in text:
                    ...
                    if keyword: return fmt_df(search_stocks(keyword))
            return "请提供搜索关键词"
